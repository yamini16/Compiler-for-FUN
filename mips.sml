...
(* caller-save registers, the return-address register, and the return-result register ($v0) *)
   | Jal(_) => {def=list2set(reg "$v0"::reg "$ra"::callerSaved), use=list2set[reg "$a0"]}
   | J(_) => {def=RegSet.empty,use=RegSet.empty} (* Wasn't given in the template! *)
